export class TagManager {
    ...
    public add(...tags: string[]) {
        ...
        tags.forEach(tag => {
            // Gestion des erreurs
            if (!pattern.test(tag)) {
                new Error(`The tag must respect the following criteria: any combination of lowercase letters, hyphens (not at the beginning or at the end) and digits (tag: "${tag}")`);
            }
